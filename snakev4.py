...
Dificultad: {dificultad}. Genera exactamente {num_obs} obstaculos en posiciones multiplos de 20.
...
def detectar_causa_muerte(cab, serpiente, obstaculos):
    if cab[0] < 0 or cab[0] >= 480 or cab[1] < 40 or cab[1] >= 400:
        return "borde"
    if cab in obstaculos:
        return "obstaculo"
    return "cuerpo"

def pedir_primera_direccion(e, memoria):
    ventana.fill((0, 30, 0))
...
def pedir_direccion_ia(e, memoria):
    if e["ia_pensando"]:
        return
    e["ia_pensando"] = True
...
while True:
    ...
    if estado["modo"] == "ia" and not estado["ia_lista"]:
        pedir_primera_direccion(estado, memoria)
        continue
    ...
    if estado["modo"] == "ia":
        ...
        if estado["ia_contador"] >= 3:
            ...
            if estado["movimientos_sin_comer"] < 7:
                pedir_direccion_ia(estado, memoria)
            else:
